...
class TestContributionSchedule(unittest.TestCase):
    ...
    def test_contribute_none_schedule(self):
        co = NoneSchedule()
        amount = co.get_contributions_until(date(1905, 2, 28))
    ...
    def test_contributions_no_effect_until_next_date(self):
        for schedule in [MonthlySchedule, SemiMonthlySchedule, BiweeklySchedule]:
            ...
            amounts = s.get_contributions_until(date(1850, 1, 1))
            self.assertEqual(0, self.add_up(amounts))
            amounts = s.get_contributions_until(date(1899, 12, 31))
            self.assertEqual(0, self.add_up(amounts))
            amounts = s.get_contributions_until(date(1900, 1, 1))
            ...
            amounts = s.get_contributions_until(date(1700, 4, 17))
            self.assertEqual(0, self.add_up(amounts))

        s = SemiMonthlySchedule(1, date(1900, 1, 15))
        amounts = s.get_contributions_until(date(1900, 1, 15))
        self.assertEqual(1, self.add_up(amounts))
    ...
    def test_one_time_contributions_across_types(self):
        for schedule in [MonthlySchedule, SemiMonthlySchedule, BiweeklySchedule]:
            ...
            received = []
            received += s.get_contributions_until(date(1900, 1, 1))
            self.assertEqual(0, self.add_up(received), f"While testing {schedule.__name__}")
            received += s.get_contributions_until(date(1900, 1, 2))
    ...
    def test_one_contribution_ordering(self):
        ...
        received += s.get_contributions_until(date(1900, 1, 9))
        self.assertEqual(11, self.add_up(received))
        received += s.get_contributions_until(date(1900, 2, 1))
    ...
    def test_get_next_contribution_date_with_one_times_added(self):
        ...
        received += s.get_contributions_until(date(1900, 2, 2))
    ...
    def test_monthly_schedule(self):
        ...
        received += s.get_contributions_until(date(1900, 1, 3))
        self.assertEqual(start_date-timedelta(days=31), s.last_contribution_date)
        self.assertEqual(start_date, s.get_next_contribution_date())
        self.assertEqual(0, self.add_up(received))  # Nothing contributed yet

        received += s.get_contributions_until(date(1900, 1, 4))  # Monthly pay day reached
        self.assertEqual(date(1900, 1, 4), s.last_contribution_date)
        self.assertEqual(date(1900, 2, 4), s.get_next_contribution_date())
        self.assertEqual(1, self.add_up(received))

        received += s.get_contributions_until(date(1900, 2, 3))  # next month before pay day
        self.assertEqual(date(1900, 1, 4), s.last_contribution_date)
        self.assertEqual(date(1900, 2, 4), s.get_next_contribution_date())
        self.assertEqual(1, self.add_up(received))

        received += s.get_contributions_until(date(1900, 2, 4))  # 2nd monthly comes in
        ...
        received += s.get_contributions_until(date(1901, 2, 4))  # 1 year later
        ...
        received += s.get_contributions_until(date(1901, 2, 14))
        self.assertEqual(date(1901, 2, 4), s.last_contribution_date)
        self.assertEqual(date(1901, 3, 4), s.get_next_contribution_date())
        self.assertEqual(2 + 12, self.add_up(received))
    ...
    def test_biweekly_schedules(self):
        ...
        received += cbw.get_contributions_until(date(1900, 1, 3))  # One day just before 1st biweekly
        self.assertEqual(start_date-timedelta(days=14), cbw.last_contribution_date)
        self.assertEqual(0, self.add_up(received))  # Nothing contributed yet

        received += cbw.get_contributions_until(date(1900, 1, 4))  # Biweekly is the first to pay
        ...
        received += cbw.get_contributions_until(date(1900, 1, 15))  # First semi-monthly
        self.assertEqual(date(1900, 1, 4), cbw.last_contribution_date)
        self.assertEqual(cbw.last_contribution_date + timedelta(days=14), cbw.get_next_contribution_date())
        self.assertEqual(1, self.add_up(received))

        received += cbw.get_contributions_until(date(1900, 2, 1))  # Semi-monthly and Bi-weekly both hit on Feb 1
        self.assertEqual(date(1900, 2, 1), cbw.last_contribution_date)
        self.assertEqual(cbw.last_contribution_date + timedelta(days=14), cbw.get_next_contribution_date())
        self.assertEqual(3, self.add_up(received))

        received += cbw.get_contributions_until(date(1900, 2, 4))  # No change
        self.assertEqual(date(1900, 2, 1), cbw.last_contribution_date)
        self.assertEqual(cbw.last_contribution_date + timedelta(days=14), cbw.get_next_contribution_date())
        self.assertEqual(3, self.add_up(received))

        received += cbw.get_contributions_until(date(1901, 2, 4))  # 1 year later
        ...
        received += cbw.get_contributions_until(date(1901, 2, 14))
    ...
    def test_semi_monthly_schedules(self):
        ...
        received += csm.get_contributions_until(date(1900, 1, 4))  # Go to the start date, but not 1st nor 15th
        self.assertEqual(date(1900, 1, 1), csm.last_contribution_date)
        self.assertEqual(date(1900, 1, 15), csm.get_next_contribution_date())
        self.assertEqual(0, self.add_up(received))

        received += csm.get_contributions_until(date(1900, 1, 15))  # First semi-monthly
        self.assertEqual(date(1900, 1, 15), csm.last_contribution_date)
        self.assertEqual(date(1900, 2, 1), csm.get_next_contribution_date())
        self.assertEqual(1, self.add_up(received))

        received += csm.get_contributions_until(date(1900, 2, 1))  # Next semi-monthly
        self.assertEqual(date(1900, 2, 1), csm.last_contribution_date)
        self.assertEqual(date(1900, 2, 15), csm.get_next_contribution_date())
        self.assertEqual(2, self.add_up(received))

        received += csm.get_contributions_until(date(1900, 1, 15))  # try to hit last date, in the past
        self.assertEqual(date(1900, 2, 1), csm.last_contribution_date)
        self.assertEqual(2, self.add_up(received))

        received += csm.get_contributions_until(date(1900, 2, 4))  # No change
        self.assertEqual(date(1900, 2, 1), csm.last_contribution_date)
        self.assertEqual(date(1900, 2, 15), csm.get_next_contribution_date())
        self.assertEqual(2, self.add_up(received))

        received += csm.get_contributions_until(date(1900, 3, 16))  # Change date's month and half it lies in
        ...
        received += csm.get_contributions_until(date(1901, 2, 4))  # 1 year later
        ...
        # Move a few more days to get the next biweekly on Thursday 1901, 2, 7
        received += csm.get_contributions_until(date(1901, 2, 14))
        self.assertEqual(date(1901, 2, 1), csm.last_contribution_date)
        self.assertEqual(date(1901, 2, 15), csm.get_next_contribution_date())
        self.assertEqual(2 + 24, self.add_up(received))
